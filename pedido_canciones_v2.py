...
CATALOGO = {
    "Alejandro Fernández": {
        "Me Dediqué a Perderte": os.path.join(CARPETA_IMAGENES, "alejandro_perderte.jpg"),
        "Se Me Va la Voz": os.path.join(CARPETA_IMAGENES, "alejandro_voz.jpg"),
        "Nube Viajera": os.path.join(CARPETA_IMAGENES, "alejandro_nube.jpg")
    },
    "Shakira": {
        "Waka Waka (This Time for Africa)": os.path.join(CARPETA_IMAGENES, "shakira_waka.jpg"),
        "Ciega, Sordomuda": os.path.join(CARPETA_IMAGENES, "shakira_ciega.jpg"),
        "Te Felicito": os.path.join(CARPETA_IMAGENES, "shakira_tefelicito.jpg")
    },
    "Luis Miguel": {
        "La Incondicional": os.path.join(CARPETA_IMAGENES, "luismi_incondicional.jpg"),
        "Hasta Que Me Olvides": os.path.join(CARPETA_IMAGENES, "luismi_olvides.jpg"),
        "Culpable o No": os.path.join(CARPETA_IMAGENES, "luismi_culpable.jpg")
    }
}
...
RUTA_IMAGEN_POR_DEFECTO = os.path.join(CARPETA_IMAGENES, "default_album.png") 
...
def realizar_pedido():
    ...
    # Insertar con o sin imagen dependiendo si se pudo cargar
    if miniatura_tk:
    ...
    # 4. Limpiar formulario y preparar para siguiente pedido
    OYENTE_NOMBRE.set("")
    INTERPRETE_SELECCIONADO.set("")
    CANCION_SELECCIONADA.set("")
    COMBO_CANCION['values'] = []
    COMBO_CANCION['state'] = 'disabled'
...
def eliminar_pedido_seleccionado():
    """Elimina el pedido seleccionado de la cola."""
    global LISTA_PEDIDOS_COLA
    
    # Obtener el item seleccionado en el Treeview
    seleccion = TREE_COLA.selection()
    
    if not seleccion:
        messagebox.showwarning("Atención", "Por favor selecciona un pedido de la cola para eliminar.")
        return
    
    item_id = seleccion[0]
    
    # Buscar el pedido en la lista interna
    pedido_encontrado = None
    indice = -1
    
    for i, pedido in enumerate(LISTA_PEDIDOS_COLA):
        if pedido['id'] == item_id:
    ...
    if pedido_encontrado:
        ...
        confirmacion = messagebox.askyesno(
            ...
            f"¿Deseas eliminar este pedido?\n\n"
            f"🎵 {pedido_encontrado['cancion']} ({pedido_encontrado['artista']})\n"
            f"👤 por {pedido_encontrado['oyente']}"
        )
        
        if confirmacion:
            # Eliminar de la lista interna
            LISTA_PEDIDOS_COLA.pop(indice)
            
            # Eliminar del Treeview
            TREE_COLA.delete(item_id)
            
            messagebox.showinfo("Eliminado", "El pedido ha sido eliminado de la cola.")
...
def crear_interfaz_principal(master):
    ...
    # Frame para botones de control de cola
    frame_botones = ttk.Frame(master)
    frame_botones.grid(row=6, column=0, columnspan=2, pady=5)
    
    # BOTÓN PARA ELIMINAR PEDIDO SELECCIONADO
    ttk.Button(frame_botones, 
               text="Eliminar Pedido Seleccionado 🗑️", 
...
style.configure('Warning.TButton', foreground='black', background='#ffc107', font=('Arial', 10, 'bold'))
